## Standalone Tool & Pipeline Data Generator (v0.8)
# This is the primary calculation engine for pipeline_02_cache.py.
# Generates 3D heatmap data, unified 1D/2D statistics, and
# 1D context-dependent histograms for a pair of invariants.
# v0.8: Now requires a 'pos' argument.
# Example (v0.8):
# python tools/generate_visualizations.py /path/to/db.db --inv1 tau_NA --inv2 tau_AC --offset 1 --res1 A --res2 C --level level_1 --pos 0
...
def _bin_axis(series, invariant_name, res_level):
    """Bins one axis to its display centres for the sparse heatmap.

    Torsion axes -> INTEGER degrees, folded to [-180, 179] (no duplicated seam).
    Non-torsion axes -> snapped to the configured resolution grid
    (RESOLUTION_BINS[res_level][type]). Returns a numpy array aligned to `series`.
    """
    vals = series.to_numpy(dtype=float)
    if invariant_name in TORSION_INVARIANTS:
        v = np.round(vals)
        return (((v + 180.0) % 360.0) - 180.0).astype(int)
    bin_width = RESOLUTION_BINS[res_level][INVARIANT_TYPES[invariant_name]]
    return np.round(vals / bin_width) * bin_width
...
def _calculate_1d_histo(data_series, invariant_name):
    ...
    bin_min, bin_max = int(limit_def['limit_min']), int(limit_def['limit_max'])

    # Integer-angle binning: round each value to the nearest whole degree and fold
    # +180 -> -180 so the centres are the integers [bin_min, bin_max - 1] with no
    # duplicated seam. (1D histos are only generated for torsions, range -180..180.)
    vals = np.round(data_series.to_numpy(dtype=float))
    span = bin_max - bin_min                       # 360 for torsions
    idx = ((vals - bin_min) % span).astype(int)    # integer bin index 0..span-1
    counts = np.bincount(idx, minlength=span)[:span]
    centers = list(range(bin_min, bin_max))        # -180 .. 179

    return {
        'bins': [int(c) for c in centers],
...
def _calculate_raw_stats(data_x, data_y, inv1, inv2, is_circular_x=False, is_circular_y=False):
    ...
    if is_circular_x:
        ...
        stats['variance_x'] = round_to_sig_figs(1 - R_x)
    ...
    if is_circular_y:
        ...
        stats['variance_y'] = round_to_sig_figs(1 - R_y)
...
def generate_visualization_data(db_path, inv1, inv2, offset, res1, res2, res_level, pos, TORSION_INVARIANTS_LIST):
    """
    ...
    # v0.8 change - pass 'pos' to the query function
    df = query_data_for_comparison(db_path, inv1, inv2, offset, res1, res2, pos)
    ...
    if res_level:
        # Per-axis binning -> sparse points. Torsion axes use INTEGER-degree
        # centres (see _bin_axis); non-torsion axes keep their configured
        # resolution grid. Grouping replaces np.histogram2d so the stored
        # centres are exactly the bin labels (no half-bin offsets).
        bx = _bin_axis(data_x, inv1, res_level)
        by = _bin_axis(data_y, inv2, res_level)
        grid = pd.DataFrame({'x': bx, 'y': by}).groupby(['x', 'y']).size().reset_index(name='c')

        if not grid.empty:
            peak_row = grid.loc[grid['c'].idxmax()]
            stats_data['peak_x'] = round_to_sig_figs(float(peak_row['x']))
            stats_data['peak_y'] = round_to_sig_figs(float(peak_row['y']))
            stats_data['peak_freq'] = int(peak_row['c'])
            points_list = [(float(r.x), float(r.y), int(r.c)) for r in grid.itertuples(index=False)]
        else:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--level", type=str, default=None, choices=RESOLUTION_LEVELS, help="Resolution level for heatmap. If None, 3D data is not generated.")
    # v0.8 change - pos argument
    parser.add_argument("--pos", type=int, default=0, choices=[0, 1], help="Position focus (0 for i, 1 for i+n). Only used if offset > 0.")
    
    args = parser.parse_args()

    # v0.8 pod logic
    if args.offset == 0 and args.pos != 0:
        print("Warning: Offset is 0, forcing position (pos) to 0.")
        args.pos = 0
        
    if args.level is None:
        print("Info: No --level specified. 3D heatmap data and binned stats will not be generated.")

    print(f"Generating data for {args.inv1} vs {args.inv2} (Offset +{args.offset}, {args.res1}-{args.res2}, pos {args.pos}, {args.level} res)")

    heatmap, stats, histo_x, histo_y = generate_visualization_data(
        args.db_path, args.inv1, args.inv2, args.offset,
        args.res1, args.res2, args.level, args.pos, TORSION_INVARIANTS
    )
    ...
    output_file = f"vizdata_{args.inv1}_vs_{args.inv2}+{args.offset}_{args.res1}_{args.res2}_{args.pos}_{args.level}.json"
