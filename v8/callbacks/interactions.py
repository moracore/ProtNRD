...
def create_stats_csv(panel_state: dict, use_sci_notation: bool = False) -> str:
    ...
    for key, friendly_name in stat_order:
        ...
        # Skip stats with no value (e.g. median/min/max/covariance/pearson on
        # circular axes) instead of emitting empty "N/A" rows.
...
def register_interaction_callbacks(app):
    ...
    @app.callback(
        ...
        State('pairwise-db-dropdown', 'value')
    )
    def load_state_from_url(search, current_store_json, db_choice):
        ...
        try:
            ...
            import sqlite3; from ..constants import resolve_db

            print(f"[V8 DEBUG] Parsing Q8 URL Parameter segments: {segments}")
            with sqlite3.connect(f"file:{resolve_db(db_choice)}?mode=ro", uri=True) as conn:
    ...
    def update_share_url(panel_states_json):
        ...
        for i in range(max_idx + 1):
            ...
            res1 = state.get('res1', 'Any')
            res2 = state.get('res2', 'Any')
    ...
    app.clientside_callback(
        ...
        function(n_clicks, url_value) {
            ...
            function flash(msg, ms) {
                ...
                el.innerText = msg;
                setTimeout(function() {
                    el.innerHTML = '<i class="bi bi-link-45deg me-1"></i>Share Layout';
                }, ms);
            }

            // Legacy fallback for insecure (http://) origins. navigator.clipboard
            // is undefined unless the page is a secure context (https, localhost,
            // or 127.0.0.1), which is exactly the case on many on-campus hosts
            // served over plain HTTP. execCommand('copy') still works there.
            function legacyCopy() {
                try {
                    ...
                    flash(ok ? '✓ Copied!' : '✗ Press Ctrl+C', ok ? 1500 : 2500);
                } catch (e) {
                    flash('✗ Press Ctrl+C', 2500);
                }
            }
    ...
    @app.callback(
        ...
        State('scale-switch', 'value'), State('smooth-switch', 'value'),
        prevent_initial_call=True
    )
    def open_focus_modal(focus_clicks, panel_states_json, sci_pref, log_scale, smooth):
        log_scale = bool(log_scale); smooth = bool(smooth)
    ...
    def download(clicks, states_json, sci_pref, log_scale, smooth):
        log_scale = bool(log_scale); smooth = bool(smooth)
    ...
    def generate_panel_data(n_clicks, active_panel, inv1, inv2, offset, res1, res2, pos0_val, x_min, x_max, y_min, y_max, log_scale, colormap, states_json, db_choice):
    ...
    def render_all_panels(panel_states_json, log_scale, smooth, sci_pref):
        ...
        for i in range(MAX_GRAPHS):
            try:
                ...
                if view == 'graph' and job == '3D_HEATMAP' and state.get('stats'):
                    s = state['stats']
                    pop = f"{int(s.get('population')):,}" if s.get('population') is not None else 'N/A'
