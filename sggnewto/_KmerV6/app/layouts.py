...
def build_config_panel():
    ...
    return html.Div(
        ...
        children=[
            ...
            dcc.Dropdown(
                id='inv1-dropdown', 
                options=[{'label': INVARIANT_SHORTHAND.get(i, i), 'value': i} for i in INVARIANT_ORDER], 
                value='tau_NA'
            ),
            ...
            dcc.Dropdown(
                id='offset-dropdown', 
                options=[{'label': f'+{i}', 'value': i} for i in range(5)], 
                value=0
            ),
            ...
            dcc.Dropdown(
                id='res1-dropdown', 
                options=[{'label': res, 'value': res} for res in RESIDUE_CONTEXTS], 
                value='Any'
            ),
...
def main_layout():
    ...
    # --- NEW: Define the panel-view-stores here ---
    # This list comprehension creates all the dcc.Store components needed
    panel_view_stores = [
        dcc.Store(
            id={'type': 'panel-view-store', 'index': i}, 
            storage_type='session', 
            data='graph' # Default to graph view
    ...
    main_panel = html.Div(
        ...
        children=[
            ...
            dcc.Store(id='graph-job-store'), # Retained from original
            dcc.Store(id='status-message-store'),
            
            # Add the new stores to the layout
            *panel_view_stores, 
