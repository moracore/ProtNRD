## Pipeline Step 1: Joined Table Generation
## Creates storage-efficient, non-redundant pre-joined database 
## tables required for efficient sliding-window pairwise analysis.
## It reads from 'invariants_filtered' and generates:
## - v8_offset_1: (i, i+1)
## - v8_offset_2: (i, i+2)
## - v8_offset_3: (i, i+3)
## - v8_offset_4: (i, i+4)
...
def main(db_path):
    ...
    print("--- Pipeline Step 1 (v8): Creating Non-Redundant Joined Tables ---")
    ...
    with sqlite3.connect(db_path, timeout=3600) as conn:
        ...
        for offset in range(1, 5): # Future: un-hard-code the range
            table_name = f'v8_offset_{offset}'
            ...
            """ ##### Future: have table name be in argument
