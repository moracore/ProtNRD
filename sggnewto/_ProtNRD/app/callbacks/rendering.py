...
def create_3D_figure(data, title, uirevision_key, log_scale, colormap, inv1_name=None, inv2_name=None, x_lims=None, y_lims=None):
    ...
    cycle_range = 360
    final_x_data, final_y_data, final_z_data = original_x_data.copy(), original_y_data.copy(), original_z_data.copy()
...
def build_3d_stats_overlay(stats_data):
    ...
    pop = stats_data.get('population', 0); peak_x = stats_data.get('peak_x', 'N/A'); peak_y = stats_data.get('peak_y', 'N/A');
    return html.Div([
...
def build_full_stats_table(panel_state, use_sci_notation=False):
    """ Builds the full v8 stats table for the focus modal. """
    stats = panel_state.get('full_v8_stats') # v0.8: Updated
    inv1 = panel_state.get('inv1'); inv2 = panel_state.get('inv2');
    inv1_label = INVARIANT_SHORTHAND.get(inv1, inv1); inv2_label = INVARIANT_SHORTHAND.get(inv2, inv2);
    ...
    pair_stats = dbc.ListGroup([
        ...
        # dbc.ListGroupItem(f"Pearson's (ρ): {format_stat_value(stats.get('pearson_correlation'), use_sci_notation, precision=4)}"), # v0.8: Added Pearson
        ...
        for i in range(MAX_GRAPHS):
            ...
            if current_view == 'graph':
                ...
                flipper_button = dbc.Button(
                    ...
                    id={'type': 'toggle-view-button', 'index': i}, 
                    size="sm", title=title,
                    disabled=(job_type == '1D_STATS_VS_STATS') # Disable for stats-only
                )
