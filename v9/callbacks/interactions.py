...
def register_interaction_callbacks(app):
    ...
    @app.callback(
        ...
        State('trimer-db-dropdown', 'value'),
        prevent_initial_call=True
    )
                ...
                with sqlite3.connect(f"file:{resolve_db(db_choice)}?mode=ro", uri=True) as conn:
                    for i in range(6):
                        ...
                        try:
                            ...
                            title = f"{INVARIANT_AXIS_LABEL.get(inv1, inv1)} vs {INVARIANT_AXIS_LABEL.get(inv2, inv2)} | {triplet_key} | Focus: Pos {focus_pos} ({focus_char})"
                ...
                with sqlite3.connect(f"file:{resolve_db(db_choice)}?mode=ro", uri=True) as conn:
                    for i, seg in enumerate(segments):
                        ...
                        try:
                            ...
                            title = f"{INVARIANT_AXIS_LABEL.get(inv1, inv1)} vs {INVARIANT_AXIS_LABEL.get(inv2, inv2)} | {res1}{res2}{res3} | Focus: Pos {focus_pos} ({focus_char})"
    ...
    def open_focus_modal(focus_clicks, panel_states_json, sci_notation_pref, log_scale, smooth):
        triggered_id_dict = ctx.triggered_id
    ...
    def download_graph_html(download_clicks, panel_states_json, sci_notation_pref, log_scale, smooth):
        triggered_id_dict = ctx.triggered_id
